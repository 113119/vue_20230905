<?php
 // 'testdb' 改$aaa  id21255447_localhost
...
// password: 密碼(不得為空白，必須存在)
    ...
    if(isset($_POST["username"]) && isset($_POST["password"])){
        if($_POST["username"] != "" && $_POST["password"] != ""){
            ...
            $result = execute_sql($conn, $aaa, $sql);

            if($_POST["username"] == 'owner2'&& $_POST["password"] == '12345678'){
                if(mysqli_num_rows($result) == 1){
                    ...
                    if(execute_sql($conn, $aaa, $sql)){
                        ...
                        $result = execute_sql($conn, $db_name, $sql);
